...
class SessionStateListenerImplTest {
    ...
    void setUp() {
        this.gateway = Gateway.builder()
                ...
                .protocol("SMPP")
                .build();
        this.sessionStateListener = new SessionStateListenerImpl(gateway, socketSession, redisManager, sessions, null, null, false);
    }
    ...
    void onStateChangeWhenBoundSessionsThenSuccess() {
        ...
        verify(redisManager).hset("gateways", String.valueOf(gateway.getNetworkId()), gateway.toString());
    ...
    void onStateChangeWhenStoppedAndSuccessSessionIsZeroThenSuccess() {
        ...
        assertEquals(0, gateway.getSuccessSession());
        assertEquals("STARTED", gateway.getStatus());
        verify(redisManager, never()).hset("gateways", String.valueOf(gateway.getNetworkId()), gateway.toString());
